...
runWaypointMission(uint8_t numWaypoints, int responseTimeout)
{
  ...
  float32_t start_alt = 10;//改掉increament，可是只能走正边型
...
setWaypointInitDefaults(dji_sdk::MissionWaypointTask& waypointTask)
{
  ...
  waypointTask.idle_velocity      = 5;//怠速转动
  waypointTask.action_on_finish   = dji_sdk::MissionWaypointTask::FINISH_NO_ACTION;//结束的时候没有任何动作
  waypointTask.mission_exec_times = 1;//执行时间
  waypointTask.yaw_mode           = dji_sdk::MissionWaypointTask::YAW_MODE_AUTO;//设置角速度
  waypointTask.trace_mode         = dji_sdk::MissionWaypointTask::TRACE_POINT;//跟踪模式
  waypointTask.action_on_rc_lost  = dji_sdk::MissionWaypointTask::ACTION_AUTO;
  waypointTask.gimbal_pitch_mode  = dji_sdk::MissionWaypointTask::GIMBAL_PITCH_FREE;//万向
}
...
{//长出航点
  ...
  float64_t extAngle = 2 * M_PI / num_wp;//外角//默认为五边形时为72度
  ...
  {
    WayPointSettings  wp;
    WayPointSettings* prevWp = &wp_list[i - 1];//取出前一个点
    setWaypointDefaults(&wp);
    wp.index     = i;//编号
    wp.latitude  = (prevWp->latitude + (increment * cos(i * extAngle)));//设置高度经度纬度//为什么要乘一个increment
    wp.longitude = (prevWp->longitude + (increment * sin(i * extAngle)));
    wp.altitude  = (prevWp->altitude + 1);
    wp_list.push_back(wp);//把这个点push进去，成为下一个点的pre
  }

  // Come back home
  start_data->index = num_wp;
  wp_list.push_back(*start_data);//回到原点
...
{
  dji_sdk::MissionWaypoint waypoint;//重新来个任务点点
       ...
       wp != wp_list.end(); ++wp)
  {
    ...
    waypointTask.mission_waypoint.push_back(waypoint);//把点点塞进航点集吗？
  }
}
...
runHotpointMission(int initialRadius, int responseTimeout)
{
  ...
  dji_sdk::MissionHotpointTask hotpointTask;//绕圈圈运动

...
Mission square_mission;//这里的mission是什么，任务类？？
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber gpsSub      = nh.subscribe("dji_sdk/gps_position", 10, &gps_callback);//订了gps的回调函数
  ros::Subscriber flightStatusSub = nh.subscribe("dji_sdk/flight_status", 10, &flight_status_callback);
  ros::Subscriber displayModeSub = nh.subscribe("dji_sdk/display_mode", 10, &display_mode_callback);
  ros::Subscriber localPosition = nh.subscribe("dji_sdk/local_position", 10, &local_position_callback);

  // Publish the control signal
  ctrlPosYawPub = nh.advertise<sensor_msgs::Joy>("dji_sdk/flight_control_setpoint_ENUposition_yaw", 10);//发布控制指令
  ...
  bool obtain_control_result = obtain_control();//获得控制权，并且起飞
  bool takeoff_result;
  ...
  else
  {
    ...
    takeoff_result = monitoredTakeoff();//A3和N3的起飞方式
  }

  if(takeoff_result)
  {
    square_mission.reset();
    square_mission.start_gps_location = current_gps;//保存起飞悬停点的GPS目标，
    square_mission.start_local_position = current_local_pos;//和上面那个
    square_mission.setTarget(0, 20, 3, 60);//设置第一个目标点，怎么是四维的//角
    square_mission.state = 1;//在前往第一个目标的路上//第二个目标呢//路呢
                         ...
                         sensor_msgs::NavSatFix& origin)
{
  ...
  double deltaLat = target.latitude - origin.latitude;//转化成当地坐标
...
void Mission::step()
{
  ...
  localOffsetFromGpsOffset(localOffset, current_gps, start_gps_location);//localoffset是大地坐标下无人机当前位置相对于起始点到坐标

  double xOffsetRemaining = target_offset_x - localOffset.x;//
  double yOffsetRemaining = target_offset_y - localOffset.y;//起始点重设
  ...
  if (break_counter > 50)//认为无人机到达一个目标点
  {
    ROS_INFO("##### Route %d finished....", state);
    finished = true;
    return;
  }
  else if(break_counter > 0)//break——counter只有无人机处于目标附近的阈值内才会累加
  {
    ...
    uint8_t flag = (DJISDK::VERTICAL_VELOCITY   |
                ...
                DJISDK::STABLE_ENABLE);//要控制的维度
    controlVelYawRate.axes.push_back(0);
    controlVelYawRate.axes.push_back(0);
    controlVelYawRate.axes.push_back(0);
    controlVelYawRate.axes.push_back(0);//不用矫正x，y，z和角度吗
    controlVelYawRate.axes.push_back(flag);//输入要控制的维度
